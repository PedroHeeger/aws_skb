...
### Objective:
Neste laboratório prático, o **JupyterLab** do **Amazon SageMaker Studio** foi utilizado para treinar dois modelos de machine learning (ML) a partir de um conjunto de dados armazenado em um bucket do **Amazon S3**. Esse conjunto de dados foi o mesmo utilizado no laboratório [curso_spl_014](https://github.com/PedroHeeger/aws_skb/tree/main/self_paced_lab/curso_spl_014) na etapa de preparação. O objetivo agora foi realizar uma classificação binária, prevendo se os valores da coluna de renda (`income`) da base de dados seriam maiores ou menores que US$ 50.000, a fim de determinar se a pessoa tinha direito ou não a um programa de assistência governamental.

Para o primeiro modelo, foi utilizado o algoritmo **Extreme Gradient Boosting (XGBoost)** integrado ao SageMaker, com o código de treinamento do **XGBoost** já predefinido. No segundo modelo, explorou-se um cenário mais avançado, no qual o **XGBoost** foi executado em uma estrutura personalizada, oferecendo maior flexibilidade na customização dos scripts de treinamento e inferência. Nesse cenário, técnicas como validação cruzada k-fold puderam ser implementadas. Além disso, no segundo modelo, o ajuste dos hiperparâmetros foi realizado por meio de intervalos, permitindo a comparação dos resultados com o primeiro modelo, que utilizou parâmetros fixos. O **XGBoost** é amplamente reconhecido por sua eficiência em aprendizado baseado em árvores de decisão.

O treinamento dos modelos foi realizado a partir de arquivos **Python** no **JupyterLab**, utilizando imagens de container **Docker** que já possuíam o **XGBoost** instalado, armazenadas no **Amazon Elastic Container Registry (ECR)**, dentro de um repositório gerenciado pelo SageMaker. Após o treinamento, os modelos finais foram salvos em outro bucket do **Amazon S3**, possibilitando seu uso em futuras inferências. Os relatórios de treinamento do **XGBoost** também foram salvos nesse bucket e analisados para comparar os resultados das métricas Precision, Recall e F1-score entre os dois modelos, com intuito de identificar as diferenças de desempenho.
...
Note que nesta tarefa 1, trabalho de tuning (ajuste) foi definido com hiperparâmetros fixos. Na tarefa seguinte, o job de ajuste foi definido através de intervalos de hiperparâmetros para que o modelo buscasse a melhore combinação possível. A explicação de cada parâmetro utilizado é apresentada a seguir:
...
Com o comando seguinte foram definidas as configurações do trabalho de ajuste (tuning) de hiperparâmetros: prefixo do nome do trabalho de treinamento, prefixo do nome do trabalho de tuning, nome do modelo construído com **XGBoost**, quantidade e tipo de instância do **Amazon EC2** utilizada para executar o treinamento. O tuning job é responsável por testar diferentes combinações de hiperparâmetros para encontrar a melhor configuração para o modelo. A melhor escolha de combinações de hiperparâmetros proporcionaria ao final um resultado do F1-score melhor, indicando um modelo melhor, com poucos falsos positivos e falsos negativos. O trabalho de treinamento acontecia dentro do job de tuning.
