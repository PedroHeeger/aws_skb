...
- [Lab - Serverless Web Apps using Amazon DynamoDB - Part 3](../curso_spl_048/): O último lab consistiu em provisionar uma API no **Amazon API Gateway** para publicação do aplicativo na web, e testar a aplicação acessando e intergaindo com ela. 
...
- `Route 1`:
    - `Route name` (Nome do recurso): `/getheroeslist`.
    - `Method` (Método): `POST`.
    - `Lambda Function` (Função Lambda): `getheroeslist`.
    - `CORS`: `Enable CORS` (Habilitar CORS):
        - `Gateway Responses` (Respostas de Gateway): foram marcadas as caixas `Default 4XX` e `Default 5XX`. Isso permitia que fosse visualizados os erros 4xx e 5xx no navegador caso precisasse solucionar problemas.
        - `Access-Control-Allow-Methods` (Métodos de permissão de controle de acesso): foram marcadas as caixas `OPTIONS` e `POST`.
- `Route 2`:
    - `Route name` (Nome do recurso): `/getmissiondetails`.
    - `Method` (Método): `POST`.
