...
try:
    ...
    from identity_variables import aws_profile, region, iam_group_config, iam_policy_config
...
def Show_Menu():
    print("***********************************************")
    print(" AWS - IDENTITY - PROVISIONAMENTO - IAM GROUP POLICY ")
    print("***********************************************")
    print(f"1) Adicionar a policy ({iam_policy_config['policy_name']}) ao grupo ({iam_group_config['group_name']})")
    print(f"2) Excluir a policy ({iam_policy_config['policy_name']}) do grupo ({iam_group_config['group_name']})")
    print("3) Sair")
    print("***********************************************")
    ...
    if selection == "1":
        ...
        if len(condition) > 0:
            ...
            if condition:
                ...
                print(f"Não existe a policy {iam_policy_config['policy_name']} no grupo {iam_group_config['group_name']}")
        else:
            print(f"Não existe o grupo do IAM {iam_group_config['group_name']}")
